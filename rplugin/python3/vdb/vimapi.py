...
def appendbufline(nvim: pynvim.Nvim, bufnr: int, msg: bytes, line_offset: int = 0):
    #buf: pynvim.api.Buffer = nvim.buffers.__getitem__(bufnr)
    msg = msg.split(b'\n')
    last_line: int = nvim.call('line', '$')
    #buf.append(msg)
    nvim.call('appendbufline', bufnr, last_line - line_offset, msg)
