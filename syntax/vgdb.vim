...
syn keyword cType int long char int8_t uint8_t uint16_t int16_t uint32_t int32_t int string vector uint64_t int64_t return contained
"syn match prompt "(gdb)"
...
if ! exists("b:current_syntax") || b:current_syntax != "vgdb"
    "syn match Command display "^\s*\k\+" \ contains=Cmd

    "syn match Help display "^\s*he\%[lp]\s*\k\+"
	"\ contains=Cmd

    "syn match Information display "\(^\s*inf\%[o]\s*\)\@<=\k\+" \ contains=Info

    "syn match Show display "\(^\s*sho\%[w]\s*\)\@<=\k\+" \ contains=ShwSet,Shw

    "syn match Set display "\(^\s*set\s*\)\@<=\k\+" \ contains=ShwSet,St
endif
...
syn match cIdentContained "[a-zA-Z0-9_][a-zA-Z0-9_]\+" contained
syn match cIdentContainedInParam "[a-zA-Z0-9_][a-zA-Z0-9_]\+" contained
syn match cIdentContainedInFunc "[a-zA-Z_][a-zA-Z0-9_]\+" contained
syn match LineNum '\d\+' contained
syn match Int '\d\+' contained
syn match cIdentTop "[a-zA-Z0-9_]\+"
syn match paramName "[a-zA-Z0-9_][a-zA-Z0-9_]\+" contained
syn match param "[a-zA-Z0-9_][a-zA-Z0-9_]\+=\d\+" contained contains=Int,cIdentContainedInParam
syn match param "[a-zA-Z0-9_][a-zA-Z0-9_]\+=0[xX][a-z0-9]\+" contained contains=dex,paramName
syn match paramList "[^()]*\s*,\s*[^()]*" contained contains=param,paramName
syn match funcParam "(.*)" contained contains=paramList
syn match func "[a-zA-Z0-9_]\+[ ]*(.*)" contains=cIdentContainedInFunc,Type,dex,path,funcParam
syn match StackLine "^\d\+[ ]*.*" contains=LineNum,func,cType,cIdentContained
syn match path "[^'"{}\[\]:,=() ]*\/[^/'":]\+[^'"{}\[:,]"
syn match dex "0[xX][a-z0-9]\+"
"syn match Prompt "(gdb)"
...
high def link cType Type
"high def link prompt Label
"high def link StackLine Identifier
"high def link func Identifier
high def link LineNum LineNr
high def link cIdentContained ModeMsg
high def link cIdentContainedInParam Identifier
high def link paramName Identifier
high def link cIdentContainedInFunc DiagnosticWarn
high def link path Type
high def link dex Comment

let b:current_syntax = "vgdb"
